...
namespace Nakatetsu.Track.Graph.Edge
{
    public static class TrackEdgeCompiler
    {
        ...
        /// <summary>
        /// 位置間の弦長をdoubleで累積する旧方式。微分の数値積分ではない。
        /// 各評価点を保存する。入力定義は変更せず、失敗時に部分的なLUTを返さない。
        /// </summary>
            ...
            float integrationStepM, out List<TrackEdgeDistanceSample> distanceMap, out string error)
        {
            ...
            foreach (var segment in geometry.horizontalSegments)
            {
                ...
                if (!Finite(previousEnd)) return Fail("Invalid horizontal endpoint.", out error);
                AddBoundary(boundaries, previousEnd, min, max);
            }
            if (previousEnd < max) return Fail("Horizontal segments do not cover the Edge.", out error);
            ...
            foreach (var segment in edge.offsetSegments)
            {
                ...
                {
                    if (segment.startDistanceOnGeometryM > covered) return Fail("Gap in offset coverage.", out error);
            ...
            if (covered < max) return Fail("Offset segments do not cover the Edge.", out error);

            var points = new List<float>(boundaries);
            if (start > end) points.Reverse();
            ...
            for (int interval = 1; interval < points.Count; interval++)
            {
                ...
                for (int i = 1; i <= count; i++)
                {
                    ...
                    if (!Finite(chord) || chord <= 0f) return Fail("Nonfinite or zero-length integration interval.", out error);
                    accumulated += chord;
        ...
        {
            if (!TrackEdgeCalculator.TryEvaluateAtGeometryDistance(edge, geometry, distance, out position, out var derivative)) return false;
        ...
        {
            if (value > min && value < max) boundaries.Add(value);
        }
