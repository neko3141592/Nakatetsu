...
namespace Nakatetsu.Train.Debugging.Editor
{
    ...
    public sealed class TrainDebugControllerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.HelpBox("TrainRootのある編成ルートに追加し、Play後に運転台を準備 → Pノッチで発進します。線路GraphとPresentation Spawnerが設定されている場合は車両モデルも移動します。", MessageType.Info);
            using (new EditorGUI.DisabledScope(!UnityEngine.Application.isPlaying))
            {
                if (GUILayout.Button("機器を再取得"))
                {
                    controller.RefreshReferences();
                }
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("前側運転台を準備"))
                {
                    controller.PrepareFrontCab();
                }
                if (GUILayout.Button("後側運転台を準備"))
                {
                    controller.PrepareRearCab();
                }
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.LabelField("レバーサー（選択中の運転台基準・停車かつN）");
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("後進"))
                {
                    controller.SetReverser(-1);
                }
                if (GUILayout.Button("中立"))
                {
                    controller.SetReverser(0);
                }
                if (GUILayout.Button("前進"))
                {
                    controller.SetReverser(1);
                }
                EditorGUILayout.EndHorizontal();
                ...
                {
                    if (GUILayout.Button($"B{i}"))
                    {
                        controller.SetBrake(i);
                    }
                }
                EditorGUILayout.EndHorizontal();
                if (GUILayout.Button("非常ブレーキ"))
                {
                    controller.SetEmergencyBrake();
                }
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("ドア（全車・編成前方を基準とした左右）");
                EditorGUILayout.BeginHorizontal();
                if (GUILayout.Button("左側を開く"))
                {
                    controller.OpenLeftDoors();
                }
                if (GUILayout.Button("右側を開く"))
                {
                    controller.OpenRightDoors();
                }
                if (GUILayout.Button("両側を閉じる"))
                {
            ...
            {
                EditorGUILayout.HelpBox(controller.GetStatus(), MessageType.None);
            }
        }
    }
}
