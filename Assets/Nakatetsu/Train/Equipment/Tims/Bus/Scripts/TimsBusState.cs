...
{
    [Serializable]
    public sealed class TimsBusState
    {
        private readonly Dictionary<TimsTagKey, TimsValue> tags = new();
        public IReadOnlyDictionary<TimsTagKey, TimsValue> Tags => tags;
        ...
        public void ReplaceSnapshot(
            IEnumerable<KeyValuePair<TimsTagKey, TimsValue>> snapshot)
        {
            tags.Clear();
            if (snapshot == null)
            {
            ...
            foreach (KeyValuePair<TimsTagKey, TimsValue> pair in snapshot)
            {
