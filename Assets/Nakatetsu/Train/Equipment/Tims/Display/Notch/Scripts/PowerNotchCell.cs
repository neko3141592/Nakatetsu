using TMPro;
using UnityEngine;
using UnityEngine.UI;

namespace Nakatetsu.Train.Equipment.Tims.Presentation.Indicators
{
    public enum PowerNotchCellState
    {
        On,
        Off,
        Blank
    }

    [DisallowMultipleComponent]
    public sealed class PowerNotchCell : MonoBehaviour
    {
        [Header("State")]
        [SerializeField]
        private PowerNotchCellState state = PowerNotchCellState.Off;

        [SerializeField, Min(1)]
        private int notch = 1;

        [Header("Objects")]
        [SerializeField]
        private Image background;

        [SerializeField]
        private TMP_Text pText;

        [SerializeField]
        private TMP_Text notchText;

        [Header("On Colors")]
        [SerializeField]
        private Color backgroundOnColor = Color.cyan;

        [SerializeField]
        private Color textOnColor = Color.black;

        [Header("Off Colors")]
        [SerializeField]
        private Color backgroundOffColor = Color.black;

        [SerializeField]
        private Color textOffColor = Color.gray;

        public PowerNotchCellState State => state;
        public int Notch => notch;

        private void Awake()
        {
            CacheChildComponents();
            ApplyVisuals();
        }

        private void OnValidate()
        {
            notch = Mathf.Max(1, notch);
            CacheChildComponents();
            ApplyVisuals();
        }

        public void SetState(PowerNotchCellState newState)
        {
            if (state == newState)
            {
                return;
            }

            state = newState;
            ApplyVisuals();
        }

        public void SetNotch(int newNotch)
        {
            newNotch = Mathf.Max(1, newNotch);
            if (notch == newNotch)
            {
                return;
            }

            notch = newNotch;
            ApplyVisuals();
        }

        public void Set(PowerNotchCellState newState, int newNotch)
        {
            state = newState;
            notch = Mathf.Max(1, newNotch);
            ApplyVisuals();
        }

        public void SetLabelVisible(bool visible)
        {
            CacheChildComponents();
            if (pText != null) pText.enabled = visible;
            if (notchText != null) notchText.enabled = visible;
        }

        private void ApplyVisuals()
        {
            if (background == null || pText == null || notchText == null)
            {
                return;
            }

            bool isBlank = state == PowerNotchCellState.Blank;
            bool isOn = state == PowerNotchCellState.On;

            background.color = isOn || isBlank
                ? backgroundOnColor
                : backgroundOffColor;

            Color textColor = isOn ? textOnColor : textOffColor;
            pText.color = textColor;
            notchText.color = textColor;

            pText.enabled = !isBlank;
            notchText.enabled = !isBlank;
            notchText.text = notch.ToString();
        }

        private void CacheChildComponents()
        {
            background ??= FindChildComponent<Image>("Background");
            pText ??= FindChildComponent<TMP_Text>("P_text");
            notchText ??= FindChildComponent<TMP_Text>("Notch_text");
        }

        private T FindChildComponent<T>(string childName) where T : Component
        {
            foreach (Transform child in GetComponentsInChildren<Transform>(true))
            {
                if (child.name == childName && child.TryGetComponent(out T component))
                {
                    return component;
                }
            }

            return null;
        }
    }
}
