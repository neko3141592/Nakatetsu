...
車両機器と物理シミュレーションで、Controller の入力経路を分ける。

- `IEquipmentController` は、TIMS、接点、センサーなど実車相当の入力元を
  `CollectInput` で読み取る。
- `ISimulationController` は、上位のオーケストレーターが入力を明示的に設定した後、
  `Calculate -> ApplyOutput` を実行する。自身で機器や TIMS を探索しない。
