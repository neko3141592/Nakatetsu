...
3. Gameビューのパネルで `Install doors on all cars` を押す。
...
`Assets/Nakatetsu/Train/Equipment/Door/Prefabs/Door.prefab` を各CarDefinitionの `additionalEquipmentPrefabs` に追加する。
各車1個を前提とする。既存の速度センサーPrefabは残す。
`Physical Simulation` の `doorSimulationPrefab` に
`Assets/Nakatetsu/Train/Simulation/Door/Prefabs/TrainDoorSimulation.prefab` を設定する。
TrainSimulationBuilderが物理モデルを生成し、TrainSimulationControllerが車両IndexでDoorControllerへ接続する。
既存のTc1 / M / T / Tc2のCarDefinitionには両方を設定済み。
...
DoorTestsで、Prefab/Builder/編成Simulation/速度測定/LocalBus/MasterBusを通して確認する。
...
- 制御用Door.prefabと物理モデル用TrainDoorSimulation.prefabを分離。既存のDoorTestsを新しいBuilder構成に合わせ、隔離Unityプロジェクトで17件すべて成功。
- テストの追加はなし。実際のSceneでのPlay操作は未確認。
